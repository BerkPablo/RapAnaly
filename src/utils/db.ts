...
export const getSessions = async () => {
    if (!isSupabaseConfigured) {
        const sessions = JSON.parse(localStorage.getItem('rap_sessions') || '[]');
        const hits = JSON.parse(localStorage.getItem('rap_hits') || '[]');
        return sessions.map((s: any) => ({
            ...s,
            // Count hits for only one device to reflect the session's actual shot count
            hit_count: hits.filter((h: any) => h.session_id === s.id && h.device_type === 'MLM_DS').length
        })).sort((a: any, b: any) =>
    ...
    return data.map((s: any) => ({
        ...s,
        // Count hits for only one device to reflect the session's actual shot count
        hit_count: s.hits ? s.hits.filter((h: any) => h.device_type === 'MLM_DS').length : 0
    }));
};
...
export const deleteSession = async (sessionId: string) => {
    if (!isSupabaseConfigured) {
        ...
        localStorage.setItem('rap_sessions', JSON.stringify(filteredSessions));

        const hits = JSON.parse(localStorage.getItem('rap_hits') || '[]');
        const filteredHits = hits.filter((h: any) => h.session_id !== sessionId);
        localStorage.setItem('rap_hits', JSON.stringify(filteredHits));
        return;
    }
